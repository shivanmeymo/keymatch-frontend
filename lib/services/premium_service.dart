...
class PremiumService {
  ...
  static const Map<String, dynamic> _premiumFeatures = {
    ...
    'ai_messages': {
      'name': 'AI Messages',
      'description': 'Send messages to AI matchmaker',
      'free_limit': 5,
      'free_reset_days': 1, // Reset every day
    },
    'like_by_message': {
      'name': 'Like by sending message',
      'description': 'Automatically like profiles when you send them a message',
      'premium_only': true,
    },
  };
  ...
  static Future<bool> isPremium() async {
    try {
      ...
      // Check backend API for premium status
      final backendPremiumStatus = await _getBackendPremiumStatus();
  ...
  // Get premium status from backend API
  static Future<bool?> _getBackendPremiumStatus() async {
    try {
      ...
      print('Premium status API response: ${response.statusCode} ${response.body}');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['premiumExpiry'] != null) {
          await prefs.setString(_premiumExpiryKey, data['premiumExpiry']);
        }
        
        return isPremium;
      }
    } catch (e) {
      print('Error getting backend premium status: $e');
    }
    return null;
  }
  ...
  static Future<DateTime?> getPremiumExpiry() async {
    try {
      ...
      // Check backend API for premium expiry
      final backendExpiry = await _getBackendPremiumExpiry();
  ...
  // Get premium expiry from backend API
  static Future<DateTime?> _getBackendPremiumExpiry() async {
    try {
      final token = await AuthService.getToken();
      if (token == null) return null;
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        final expiryString = data['premiumExpiry'];
        ...
        if (expiryString != null) {
          ...
          // Update local storage with backend data
          final prefs = await SharedPreferences.getInstance();
          await prefs.setString(_premiumExpiryKey, expiryString);
          
          return expiry;
        }
      }
    } catch (e) {
      print('Error getting backend premium expiry: $e');
    }
    return null;
  }
  ...
  static Future<bool> cancelPremium() async {
    try {
      final result = await BillingService.cancelSubscription();
