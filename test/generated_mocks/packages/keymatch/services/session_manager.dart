...
class SessionManager {
  ...
  static Future<void> storeSession({
    ...
    try {
      ...
      // Backup in SharedPreferences (fallback)
  ...
  static Future<bool> refreshSession() async {
    try {
      ...
      await _secureStorage.write(
        key: _sessionKey,
        value: json.encode(updatedSessionData),
      );
  ...
  static Future<Map<String, dynamic>?> _getStoredSession() async {
    try {
      ...
      String? sessionString = await _secureStorage.read(key: _sessionKey);
      
      // Fallback to SharedPreferences
      if (sessionString == null) {
        final prefs = await SharedPreferences.getInstance();
        sessionString = prefs.getString(_sessionKey);
      }
  ...
  static Future<bool> _validateSessionWithBackend(String token) async {
    try {
      final response = await http.get(
        Uri.parse('${AuthService.baseUrl}/api/auth/me'),
        headers: {
